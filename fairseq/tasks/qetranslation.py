...
class QETranslationTask(FairseqTask):
    ...
    def load_dataset2(self, split, **kwargs):
        ...
        prefix = os.path.join(data_path, split+"."+self.args.source_lang+"-"+self.args.target_lang)
        ...
        with open(prefix+"."+self.args.target_lang, encoding='utf-8') as file:
            for line in file:
                sentence = line.strip()
                tokens = Tokenizer.tokenize(sentence, self.tgt_dict, add_if_not_exist=False)
