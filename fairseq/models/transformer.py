...
#        src_tokens = src_tokens.type(torch.LongTensor).cuda(1)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        return x, {'attn': attn, 'inner_states': inner_states, 'decoder_feat':decoder_feat, 'encoder_feat': encoder_out}
...
class TransformerDecoderQE(FairseqIncrementalDecoder):
    """
    Transformer decoder consisting of *args.decoder_layers* layers. Each layer
    is a :class:`TransformerDecoderLayer`.
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        self.fc_end = nn.Linear(self.hidden_size*30, 20)
        self.fc_end_end = nn.Linear(20, 1)
        nn.init.xavier_uniform_(self.fc_end.weight)
        nn.init.xavier_uniform_(self.fc_end_end.weight)
  ...
  #      z = x
   ...
   #     h0 = torch.zeros(self.hidden_size)
    #    c0 = torch.zeros(self.hidden_size)
        h0 = torch.zeros(self.num_layers, x.shape[0],self.hidden_size).cuda(1)
        c0 = torch.zeros(self.num_layers, x.shape[0],self.hidden_size).cuda(1)
        ...
        if out.shape[1] >= 30:
            ...
            y = torch.zeros((out.shape[0], 30-out.shape[1], out.shape[2])).cuda(1)
...
@register_model_architecture('transformer_qe', 'transformer_wmt_en_de_big_qe')
def transformer_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)
